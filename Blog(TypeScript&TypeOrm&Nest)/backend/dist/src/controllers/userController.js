...
import { getUserPostsQuerySchema, updateUserSchema, userIdParamSchema, listUsersQuerySchema, } from "../validations/userValidation.js";
...
const { UNABLE_TO_FETCH_USERS, UNABLE_TO_FETCH_USER_POSTS, USER_NOT_FOUND, UNABLE_TO_UPDATE_USER, UNABLE_TO_DELETE_USER, UNABLE_TO_FETCH_CURRENT_USER, ACCESS_TOKEN_REQUIRED } = ERROR_MESSAGES;
...
export async function getUserPostsWithComment(req, res) {
    ...
    const validatedQuery = validateRequest(getUserPostsQuerySchema, req.query, res, { convert: true });
    if (!validatedQuery)
        return;
    // Joi validation ensures page and limit are always present (defaults applied)
...
export async function update(req, res) {
    ...
    try {
        ...
        const validatedBody = validateRequest(updateUserSchema, req.body, res);
        if (!validatedBody)
            return;
        ...
        const { id, name: userName, email: userEmail, phone: userPhone, image: userImage, status, } = data;
        res.status(OK).send({
            data: {
