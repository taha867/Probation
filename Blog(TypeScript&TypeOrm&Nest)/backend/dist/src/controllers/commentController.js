...
import { createCommentSchema, updateCommentSchema, listCommentsQuerySchema, commentIdParamSchema, } from "../validations/commentValidation.js";
...
const { UNABLE_TO_CREATE_COMMENT, UNABLE_TO_FETCH_COMMENT, UNABLE_TO_DELETE_COMMENT, UNABLE_TO_UPDATE_COMMENT, UNABLE_TO_FETCH_COMMENTS, COMMENT_NOT_FOUND, ACCESS_TOKEN_REQUIRED } = ERROR_MESSAGES;
const { COMMENT_CREATED, COMMENT_UPDATED, COMMENT_DELETED, } = SUCCESS_MESSAGES;
/**
...
export async function list(req, res) {
    const validatedQuery = validateRequest(listCommentsQuerySchema, req.query, res, { convert: true });
    if (!validatedQuery)
        return;
...
export async function get(req, res) {
    try {
        const validatedParams = validateRequest(commentIdParamSchema, req.params, res, { convert: true });
        if (!validatedParams)
            return;
