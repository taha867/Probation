...
export class UserService {
     ...
     */
    async getUserPostsWithComments(params) {
        ...
        const [posts, total] = await this.postRepo.findUserPostsWithComments(userId, page, limit, search);
        return {
            user,
            posts: posts, // usage of any removed
            meta: buildPaginationMeta({ total, page, limit }),
        };
    }
    /**
     ...
     */
    async updateUserForSelf(params) {
        ...
        if (fileBuffer) {
            ...
            const uploadResult = await uploadImageToCloudinary(fileBuffer, "blog/users", fileName || "profile-image");
     ...
     */
    async deleteUserForSelf(params) {
        ...
        await this.userRepo.delete(requestedUserId);
        return { ok: true };
    }
}
