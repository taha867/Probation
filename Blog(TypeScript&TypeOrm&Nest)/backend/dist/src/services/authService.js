...
const { UNPROCESSABLE_ENTITY, UNAUTHORIZED, NOT_FOUND, INTERNAL_SERVER_ERROR } = HTTP_STATUS;
...
export class AuthService {
    ...
    async registerUser(input) {
        ...
        await this.userRepo.create({
            name,
            email,
            password: await hashPassword(password), // Hashing explicitly
            phone: phone || null,
            image: image || null,
            status: LOGGED_OUT,
        });
    ...
    async authenticateUser(input) {
        ...
        const refreshToken = signToken({ userId: id, tokenVersion, type: "refresh" }, { expiresIn: "7d" });
        return {
            ok: true,
            data: {
                user: {
    ...
    async createPasswordResetToken(email) {
        ...
        const resetToken = signToken({ userId, type: "password_reset" }, { expiresIn: "1h" });
        try {
            ...
            throw new AppError("EMAIL_SEND_FAILED", HTTP_STATUS.INTERNAL_SERVER_ERROR);
        }
    }
    /**
